...
const PlantUMLVisitor = require("composer-common/lib/codegen/fromcto/plantuml/plantumlvisitor");
...
composer_common_1.Logger.setFunctionalLogger({
    log: () => {
        // none, zip, nada!
    }
});
...
const modelManager = new composer_common_1.ModelManager();
const aclManager = new composer_common_1.AclManager(modelManager);
const queryManager = new composer_common_1.QueryManager(modelManager);
...
connection.onDidChangeConfiguration((change) => {
    ...
    options = change.settings.composer;
    ...
    if (params.arguments.length >= 1) {
        if (params.arguments[0].external) {
            ...
            if (originatingFileName.length > 0) {
                ...
                const originatingFileNameUri = vscode_uri_1.default.parse(originatingFileName);
 ...
 */
function handleGenerateUml(diagramTitle, originatingFileName) {
    // setup the visitor that walks the model
    const visitor = new PlantUMLVisitor();
    const writer = new composer_common_1.Writer();
    const parameters = {
        fileWriter: writer
    };
    ...
    for (let n = 0; n < modelFiles.length; n++) {
        ...
        else if (modelFile.getNamespace() !== composer_common_1.ModelUtil.getSystemNamespace()) {
            result = result.concat(modelFile.getAllDeclarations());
        }
    }
 ...
 * @private
 */
function validateCtoModelFile(textDocument) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            documents.all().forEach((doc) => {
                if (doc.languageId === 'composer') {
                    ...
                    if (modelContent.length > 0) {
                        ...
                        try {
                            const TheModel = new composer_common_1.ModelFile(modelManager, modelContent, doc.uri);
            ...
            const aModel = new composer_common_1.ModelFile(modelManager, modelContents, textDocument.uri);
 ...
 * @private
 */
function validateNewAclModelFile(textDocument) {
    try {
 ...
 * @private
 */
function validateExistingAclModelFile(aclFile) {
    try {
 ...
 * @private
 */
function validateNewQueryModelFile(textDocument) {
    try {
 ...
 * @private
 */
function validateExistingQueryModelFile(queryFile) {
    try {
 ...
 * @private
 */
function buildAndSendDiagnosticFromException(err, lineCount, sourceURI) {
    const diagnostics = [];
    ...
    if (typeof err.getFileLocation === 'function') {
        ...
        if (index !== -1) {
            ...
            if (isNaN(curColumn) || curColumn < 0) {
                curColumn = 0;
            } // sanity check
 ...
 * @private
 */
function sendDiagnosticSuccess(sourceURI) {
    const diagnostics = [];
    // Send the computed diagnostics to VSCode. This must always be sent because:
 ...
 */
function closeTextDocument(textDocument) {
    const langId = textDocument.languageId; // type of file we are processing
    // note - this is the FULL document text as we can't do incremental yet!
    const txt = textDocument.getText();
    ...
    if (txt != null && txt.length > 0) {
        ...
        if (langId === 'composer-acl') {
            ...
            try {
                ...
                const model = new composer_common_1.ModelFile(modelManager, txt, textDocument.uri);
